...
class TopFilter extends Component {
  ...
  LayoutTwoView = (Size) => {
    ...
    if (
      !document
        .querySelector(".pgs-product-list")
  ...
  render() {
    ...
    const totalProductLength = this.props.totalProducts;
    return (
      <Row>
        <Col>
          {productlength > 0 ? (
            <p className="result-count">
              Showing 1–{productlength} results of {totalProductLength}
            </p>
          ) : (
            <p className="result-count">
              Showing 0 results of {totalProductLength}
            </p>
          )}
